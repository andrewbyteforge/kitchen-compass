    ...
    def handle(self, *args, **options):
        ...
        for key in required_keys:
            if key in ms_settings:
                ...
                if key == "CLIENT_SECRET":
                    ...
                    display_value = (
                        f"{value[:4]}...{value[-4:]}" if value else "NOT SET"
                    )
                elif key == "CLIENT_ID":
        ...
        try:
            ...
            if "client_id=" in auth_url:
                self.stdout.write(
                    self.style.SUCCESS("✓ client_id parameter found in URL")
                )
            else:
                self.stdout.write(
                    self.style.ERROR("✗ client_id parameter NOT found in URL")
                )
